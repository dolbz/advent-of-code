...
def find_possible_next_positions(current_pos, grid, step_number):
    possible_positions = list()

    # special case for first move
    if current_pos.x == 0 and current_pos.y == -1:
        possible_positions.append((Point(0, 0), step_number))
        return possible_positions

    # special case for final move
    if current_pos.x == len(grid) - 1 and current_pos.y == len(grid[0]) - 1:
        possible_positions.append(
            (Point(current_pos.x, current_pos.y + 1), step_number)
        )
        return possible_positions
...
# start off just above the grid
current_pos = Point(0, -1)
...
while True:
    ...
    for next_position, next_step_count in next_positions:
        ...
        if next_position == end_pos and next_step_count < min_step_count:
...
print(f"min steps: {min_step_count}")
